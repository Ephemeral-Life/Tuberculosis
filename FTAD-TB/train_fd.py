...
class FlowerClient(NumPyClient):
    ...
    def load_data(self):
        client_cfg = dict(
            type='CocoClassificationDataset',  # 使用自定义数据集
...
class CustomFedAvg(FedAvg):
    def __init__(self, *args, **kwargs):
        ...
        # 使用CocoClassificationDataset构建验证数据集
    ...
    def evaluate_aggregated_model(self):
        ...
        try:
            ...
            for i, output in enumerate(outputs):
                # 根据模型输出调整，假设为分类任务输出单一标签
                pred = torch.argmax(torch.tensor(output)).item()  # 假设output是logits
                gt = self.val_dataset[i]['ann_info']['labels']  # 修正为'ann_info'
                preds.append(pred)
                gts.append(gt)
